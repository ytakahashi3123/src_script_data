...
def set_parameter(config):
  ...
  #global dir_input, file_input, num_case, file_hist, su2_version, dir_result, file_result
  #global flag_cycleprocess, kind_dimension
 # global dt_cfd, n_start_cfd
  ...
  #su2_version       = config['file_names_aero']['su2_version']
  ...
  #kind_window_fft = config['fft']['kind_window']


# ---FFT routine----
def fft_routine(n_start, n_end, time_step, var, outputfile, header, kind_window):
...
# Read aerodynamic data (Su2 History data)
  ...
  num_case          = config['file_names_precicelog']['num_case']
  flag_cycleprocess = config['file_names_precicelog']['flag_cycleprocess']
  kind_dimension    = config['file_names_precicelog']['kind_dimension']
...
# ---Read displacement data---
  for i in range(0,num_case):
    ...
    time     = data_input[:,0]
    if kind_dimension == "2":
      delta_x  = data_input[:,id_precicelog_start  ]
      delta_y  = data_input[:,id_precicelog_start+1]
      force_x  = data_input[:,id_precicelog_start+2]
      force_y  = data_input[:,id_precicelog_start+3]
      coord_x0 = data_input[0,1]
      coord_y0 = data_input[0,2]
    else:
      delta_x  = data_input[:,id_precicelog_start  ]
      delta_y  = data_input[:,id_precicelog_start+1]
      delta_z  = data_input[:,id_precicelog_start+2]
      force_x  = data_input[:,id_precicelog_start+3]
      force_y  = data_input[:,id_precicelog_start+4]
      force_z  = data_input[:,id_precicelog_start+5]
      coord_x0 = data_input[0,1]
      coord_y0 = data_input[0,2]
...
#--FFT
    ...
    fft_routine( n_start_fft, n_end_fft, time_step_fft, var_fft, outputfile_fft, header_fft, kind_window_fft )
    print('End FFT routine...')

# 基準AoAで一周期とする
#--半周期の数・ステップを取得
    if(flag_cycleprocess):
      index_min = n_start_cfd
  ...
  # 周期ごとの処理
      displacement_maen_list = []
      displacement_ampl_list = []
      displacement_freq_list = []
      with open(dir_result+'/'+'log_perioddata', 'w') as f:
        for j in range(0,num_period-1):
...
#-- variables
  ...
  # --Zero padding
          ...
          print( "Min disp. [m] :", coord_min_period-coord_ref )
          print( "Max disp. [m] :", coord_max_period-coord_ref )
          print("------------")
          ...
          print( "Min disp. [m] :", coord_min_period-coord_ref, file=f )
          print( "Max disp. [m] :", coord_max_period-coord_ref, file=f )
          print("------------", file=f )

          displacement_ampl = 0.5*( coord_max_period-coord_ref - (coord_min_period-coord_ref) )
          displacement_maen = 0.5*( coord_max_period-coord_ref + (coord_min_period-coord_ref) ) 
          displacement_freq = freq_period
          displacement_ampl_list.append( displacement_ampl )
          displacement_maen_list.append( displacement_maen  )
          displacement_freq_list.append( displacement_freq)
...
#                             aerocoef_cmz[step_period_start:step_period_end]
      ...
      print( "Mean displacement-mean:",      np.mean( displacement_maen_list) )
      print( "Mean displacement-amplitude:", np.mean( displacement_ampl_list) )
      print( "Mean displacement-frequency:", np.mean( displacement_freq_list) )